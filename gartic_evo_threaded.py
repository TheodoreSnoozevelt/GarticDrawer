...
parser.add_argument('--batch', type=int, help='Number of objects to use for each batch', default=100)
parser.add_argument('--thread', type=int, help="Number of threads to use to process batches", default=1)
...
def dither(img: cv2.Mat, colors: list[tuple[int, int, int]]):
    h, w = img.shape[:2]
    bgr_colors = [(x[1], x[1], x[0]) for x in colors]
